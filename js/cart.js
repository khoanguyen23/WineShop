...
const cartCheckout = document.querySelector(".cart-checkout tbody");
...
cart.forEach((item) => {
  ...
  const cartRow2 = document.createElement("tr");

 
  cartRow.innerHTML = `
        <td class="pro-thumbnail">
            <a href="#"><img class="fit-image" src="${item.image}" alt="${item.name}"></a>
        </td>
        <td class="pro-title"><a href="#">${item.name}</a></td>
        <td class="pro-price"><span>${item.price} đ</span></td>
        <td class="pro-quantity">
            <div class="quantity">
                <div class="cart-plus-minus">
					        <input type="number" value="${item.quantity}" name="qtybutton" class="cart-plus-minus-box" id="quantityInput" data-item-name="${item.name}">
				        </div>
            </div>
        </td>
        <td class="pro-subtotal"><span>${item.price * item.quantity} đ</span></td>
        <td class="pro-remove"><a href="#" onclick="removeCartItem('${item.name}')"><i class="ti-trash"></i></a></td>
    `;
  cartRow2.innerHTML = `
        <td class="itemQuantity">${item.name}  x ${item.quantity}</td>
        <td class="text-end itemTotalPrice">${item.price * item.quantity} đ</td>

    `;
  totalQuantity += item.quantity;
  totalPrice += item.price * item.quantity;
  cartTable.appendChild(cartRow);
  cartCheckout.appendChild(cartRow2);
});

const totalRow = document.createElement("tr");
totalRow.innerHTML = `
  <td>Order Total:${totalQuantity} </td>
   <td class="text-end"> ${totalPrice} đ</td>
`;
cartCheckout.appendChild(totalRow);
...
function clearCart() {
  ...
  message.innerHTML = `
    <svg viewBox="656 573 264 182" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                ...
                                                <h3 class="text-black text-uppercase">Your cart is empty.</h3>
                                                <p class="text-black mb-5">Go to Shop Page to buy something new</p>
    `;
  cartTable.appendChild(message);
}
...
function updateCartItemQuantity(productName, newQuantity) {
  ...
  if (cartIndex !== -1) {
    ...
    const quantityInput = document.querySelector(`input[data-item-name="${productName}"]`);
    const subtotalCell = quantityInput.closest("tr").querySelector(".pro-subtotal span");
    const item = cart[cartIndex];

    quantityInput.value = newQuantity;
    subtotalCell.textContent = `${item.price * newQuantity} đ`;
...
clearCartBtn.addEventListener("click", function () {
  clearCart(); // Gọi hàm để xóa tất cả sản phẩm trong giỏ hàng
});
...
function saveOrder(orderData) {
  ...
  xhr.onload = function () {
    if (xhr.status >= 200 && xhr.status < 400) {
      ...
      console.error('Lỗi khi lưu đơn hàng:', xhr.statusText);
    }
  };
...
placeOrderButton.addEventListener('click', function () {
  ...
  if (cart.length === 0) {
    alert('Giỏ hàng của bạn đang trống. Hãy thêm sản phẩm vào giỏ hàng trước khi đặt hàng.');
    return;
  }
